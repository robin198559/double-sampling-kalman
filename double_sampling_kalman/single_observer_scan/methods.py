...
def moving_average(a, window_size=3):
    diff = np.cumsum(a, dtype=float)
    diff[window_size:] = diff[window_size:] - diff[:-window_size]
    return diff[window_size - 1 :] / window_size
...
def stop_filter_scan(error_cov_list: List[float], min_iter: int, tol: float):
    if len(error_cov_list) > min_iter:
        ...
        within_range = moving_average(signal_log, window_size=min_iter) < tol
    else:
...
def zoom_in_signal_multipliers(
    ...
    negative_convex_index_list = [
        i for i, v in enumerate(convex) if v < 0 and v < min(convex) * 0.5
    ]
    positive_convex_index_list = [
        i for i, v in enumerate(convex) if v > 0 and v > max(convex) * 0.5
    ]
