...
class DoubleKalmanOutput:
    ...
    def last_estimate(self) -> Dict[str, float]:
        return {name: value for name, value in zip(self.dependent_names, self.estimated[-1, :])}
    ...
    @property
    def diff(self):
        return pd.DataFrame((self.backward - self.forward)[:, :, 0], columns=self.dependent_names)
